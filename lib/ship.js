...
class Ship {
  constructor(canvas, ctx, type, power) {
    this.canvas = canvas;
    this.ctx = ctx;
    this.type = type;
    this.props = shipTree[type][power];
    this.shipTree = shipTree[type];
  ...
  _buildBullets() {
    ...
    for (let i = 0; i < this.props.bulletCount; i++) {
      bullets.push(new Bullet([this.props.x + (this.props.width / 2) - 2, this.props.y], this.ctx, this.canvas));
    }
  ...
  createSprite() {
    ...
    shipImage.src = this.shipTree.image;
    return new Sprite({
