...
let frame = 0;
const shipPic = Ship.createSprite(canvas);
...
const shipLeftPic = Ship.createLeftSprite(canvas);
...
const shipRightPic = Ship.createRightSprite(canvas);
...
const play = () => {
  ...
  if (gameOver(shipAttr.hp, bosses[0], stopAnime, score)) {
    return;
  } else if (isPaused) {
