...
//   .orderByChild("score").on('child_added', (snapshot) => {
//     childScore = snapshot.val();
//     document.getElementById("score-board").innerHTML += childScore.name + "-" + childScore.score + "<br>"
//   });
// }
...
const draw = () => {
  ...
  if (isPaused) {
    ...
    if (score > 10000) {
      enemy1.update([0, 1.5]);
      enemy2.update([1, 0]);
      score = enemy2.handle(enemies2, score, canvas, shipAttr, bullet);
    } else {
      enemy1.update([0, 1]);
    }
